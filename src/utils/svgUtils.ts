 ...
 */
export const isSVGFile = (file: File | null): boolean => {
 ...
 */
export const getSVGDimensions = async (svgFile: File): Promise<{ width: number; height: number; aspectRatio: number }> => {
    return new Promise((resolve) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (typeof result !== 'string') throw new Error('Invalid SVG content');

                const svgDoc = parser.parseFromString(result, 'image/svg+xml');
                const svgElement = svgDoc.documentElement;

                let width: number = SVG_CONSTANTS.DEFAULT_WIDTH;
                let height: number = SVG_CONSTANTS.DEFAULT_HEIGHT;
            ...
            try {
                ...
                if (!trimmedText.startsWith('<')) {
                    finalSvgText = SVG_XML_TEMPLATE
                        .replace(/{width}/g, (targetWidth || SVG_CONSTANTS.DEFAULT_WIDTH).toString())
                        .replace(/{height}/g, (targetHeight || SVG_CONSTANTS.DEFAULT_HEIGHT).toString())
                ...
                try {
                    ...
                    finalSvgText = SVG_XML_TEMPLATE
                        .replace(/{width}/g, originalWidth.toString())
                        .replace(/{height}/g, originalHeight.toString())
                        .replace(/{content}/g,
 ...
 */
export const createSVGPlaceholderWithAspectRatio = async (
    svgFile: File,
    targetWidth: number,
    targetHeight: number,
    format: string
 ...
 */
export const resizeSVG = async (svgFile: File, width: number, height: number): Promise<File> => {
    return new Promise((resolve, reject) => {
        const reader = new FileReader();
 ...
 * @param {File | object} svgInput - SVG file or object with file property
 * @param {number} maxSize - Maximum preview size
 * @returns {Promise<string>} Data URL preview
 */
export const generateSVGPreview = async (svgInput: File | { file: File } | any, maxSize: number = 400): Promise<string> => {
 ...
 */
export const createSVGErrorPlaceholder = async (
    image: { name: string },
    dimension: number,
    errorMessage: string
): Promise<File> => {
 ...
 */
export const createErrorPlaceholder = async (
    image: { name: string },
    width: number,
    height: number | string, // handle overload or just separate args
    title?: string,
    errorMessage?: string
): Promise<File> => {
    // Handle overload: if height is string, it's actually errorMessage (legacy simplified call)
    let actualHeight = width;
    let actualTitle = 'Error';
    let actualMessage = '';

    if (typeof height === 'string') {
        actualMessage = height;
    } else {
        actualHeight = height;
        actualTitle = title || 'Error';
    ...
    const centerY = (actualHeight as number) / 2;

    ctx.fillText(actualTitle, centerX, centerY - 15);

    ctx.font = `${Math.min(14, width / 20)}px ${FONT_CONSTANTS.DEFAULT_FONT_FAMILY}`;
    if (actualMessage && actualMessage.length > 25) {
        ctx.fillText(actualMessage.substring(0, 22) + '...', centerX, centerY + 15);
    } else {
        ctx.fillText(actualMessage, centerX, centerY + 15);
    }
