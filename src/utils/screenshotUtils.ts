 ...
 * @param {ScreenshotTemplate} template - Screenshot template
 * @returns {Object} Viewport configuration
 */
const getViewportConfig = (template: ScreenshotTemplate) => {
...
export const captureScreenshot = async (url: string, template: ScreenshotTemplate, options: CaptureOptions = {}): Promise<ScreenshotResult> => {
    try {
        ...
        if (!processedUrl.startsWith('http://') && !processedUrl.startsWith('https://')) {
            processedUrl = `${URL_CONSTANTS.DEFAULT_PROTOCOL}${processedUrl}`; // Assuming URL_CONSTANTS has DEFAULT_PROTOCOL, assuming it's imported correctly.
            // If DEFAULT_PROTOCOL is missing in URL_CONSTANTS import (it was in js file), we use hardcoded or fix constants.
            // Original js used `URL_CONSTANTS.DEFAULT_PROTOCOL` but line 167 of js says `${URL_CONSTANTS.DEFAULT_PROTOCOL}${formattedUrl}` so it must be there.
            // Wait, previous file view of screenshotUtils.js line 167 (of validateScreenshotUrlInput? No that was validationUtils).
            // screenshotUtils.js line 98: `processedUrl = `https://${processedUrl}`;` - HARDCODED HTTPS.
            // Wait, my previous view of screenshotUtils.js lines 97-99:
            // if (!processedUrl.startsWith('http://') && !processedUrl.startsWith('https://')) {
            //      processedUrl = `https://${processedUrl}`;
            // }
            // So it used https:// directly.
        }

        // Re-read screenshotUtils.js:
        // Line 2 import { URL_CONSTANTS } from '../constants';
        // But logic at line 98 uses hardcoded `https://`.
        // I will follow the original logic.
 ...
 * @returns {Promise<ScreenshotResult>} Error image data
 */
const createScreenshotErrorImage = async (template: ScreenshotTemplate, url: string, error: Error): Promise<ScreenshotResult> => {
 ...
 * @async
 * @param {ScreenshotTemplate[]} templates - Screenshot templates
 * @param {string} url - URL to capture
 * @param {CaptureOptions} options - Capture options
 * @returns {Promise<Object>} Screenshot results
 */
export async function captureMultipleScreenshots(templates: ScreenshotTemplate[], url: string, options: CaptureOptions = {}) {
 ...
 */
export function convertScreenshotResultsToImages(screenshotResults: { results: ScreenshotResult[] }): any[] {
 ...
 */
export function getDefaultScreenshotTemplates(): string[] {
 ...
 */
export function getAllScreenshotTemplateIds(): string[] {
 ...
 */
export function filterScreenshotTemplates(selectedTemplateIds: string[]): ScreenshotTemplate[] {
    if (!selectedTemplateIds || selectedTemplateIds.length === 0) {
        return [];
    }

    const allTemplates: ScreenshotTemplate[] = [];

    if (SCREENSHOT_TEMPLATES && typeof SCREENSHOT_TEMPLATES === 'object') {
        Object.values(SCREENSHOT_TEMPLATES).forEach((template: any) => {
 ...
 */
export function getScreenshotTemplateById(templateId: string): ScreenshotTemplate | null {
 ...
 */
export function getAllScreenshotTemplates(): ScreenshotTemplate[] {
    return Object.values(SCREENSHOT_TEMPLATES || {}) as ScreenshotTemplate[];
}
 ...
 */
export function getRegularScreenshotTemplates(): ScreenshotTemplate[] {
    return (Object.values(SCREENSHOT_TEMPLATES || {}) as ScreenshotTemplate[]).filter(t => !t.id.includes('-full'));
}
 ...
 */
export function getFullPageScreenshotTemplates(): ScreenshotTemplate[] {
    return (Object.values(SCREENSHOT_TEMPLATES || {}) as ScreenshotTemplate[]).filter(t => t.id.includes('-full'));
}
 ...
 */
export function getDefaultScreenshotTemplateObjects(): ScreenshotTemplate[] {
 ...
 */
export function getScreenshotTemplateConfigs(selectedTemplateIds: string[]): ScreenshotTemplate[] {
    if (!selectedTemplateIds || selectedTemplateIds.length === 0) {
        return [];
    }
 ...
 */
export function prepareScreenshotTemplates(selectedTemplateIds: string[], url: string): ScreenshotTemplate[] {
 ...
 */
export async function orchestrateScreenshotProcessing(selectedTemplateIds: string[], url: string, onProgress: ((stage: string, progress: number) => void) | null = null) {
    if (!selectedTemplateIds || selectedTemplateIds.length === 0) {
 ...
 */
export function handleScreenshotTemplateToggle(currentSelected: string[], templateId: string, isScreenshotSelected: boolean): string[] {
 ...
 */
export function calculateCaptureProgress(current: number, total: number): number {
    if (total === 0) return 0;
 ...
 */
export function createCaptureStatusMessage(successful: number, total: number, hasPlaceholders: boolean): string {
    if (successful === total) {
 ...
 */
export const getScreenshotTemplatesWithQuality = (): ScreenshotTemplate[] => {
    return (Object.values(SCREENSHOT_TEMPLATES || {}) as ScreenshotTemplate[]).map(template => {
 ...
 */
export const getDeviceViewport = (deviceType: string) => {
    switch (deviceType) {
        case 'mobile':
 ...
 */
export const getTemplateDimensions = (template: ScreenshotTemplate): string => {
 ...
 */
export const getDeviceName = (templateId: string): string => {
 ...
 * @returns {Array<string>} Array of default template IDs
 */
export const getInitialTemplates = (): string[] => {
    return ['screenshots-mobile', 'screenshots-desktop'];
};
