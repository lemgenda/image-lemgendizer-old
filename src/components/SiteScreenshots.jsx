                                ...
                                <div className="capture-results">
                                    <div className="results-header">
                                        ...
                                        <div className="text-sm font-medium px-3 py-1 bg-gray-100 rounded-full">
                                            {captureResults.successful} / {captureResults.total} successful
                                        </div>
                                    </div>

                                    {captureResults.results && captureResults.results.length > 0 && (
                                        <>
                                            <div className="results-grid">
                                                {captureResults.results.map((result, index) => (
                                                    <div
                                                        key={index}
                                                        className={`result-card ${result.success ? 'success' : 'error'}`}
                                                    >
                                                        <div className="result-header">
                                                            <i className={`${result.success ? 'fas fa-check-circle text-green-500' : 'fas fa-times-circle text-red-500'} result-icon`}></i>
                                                            <div>
                                                                <div className="font-semibold text-gray-800 capitalize">
                                                                    {result.templateName || result.device}
                                                                </div>
                                                                {result.dimensions && (
                                                        ...
                                                        {result.success ? (
                                                            <>
                                                                {renderScreenshotPreview(result)}
                                            ...
                                            {captureResults.successful > 0 && (
                                                <button
                                                    onClick={downloadAllScreenshots}
                                                    className="download-all-btn"
                                                    disabled={captureResults.successful === 0}
                                                >
                                                    <i className="fas fa-download"></i>
                                                    Download All Screenshots ({captureResults.successful} images)
                                                </button>
                                            )}
                                        </>
                                    )}
                    ...
                    {availableTemplates.length > 0 && (
                        <div className="info-note">
                            <div className="flex items-start">
                                ...
                                <div>
                                    <div className="font-semibold mb-1">Available Screenshot Templates:</div>
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        {availableTemplates.map(template => (
                                            <span
                                                key={template.id}
                                                className="px-3 py-1 bg-white/50 rounded-full text-xs font-medium border border-amber-300"
                                            >
                                                <i className={`${template.icon} mr-1`}></i>
                                                {template.name}
                                            </span>
                                        ))}
                                    </div>
